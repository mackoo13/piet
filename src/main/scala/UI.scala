...
class UI extends MainFrame {
  ...
  val program = new PietProgram()
  val codels = new Codels(4, 4, codelsArray)
  for(j <- 0 to 1) program.step()
  var i = 0
  ...
  def step() = {
    ...
    labelOut.text = "<html>OUT:<br>%s</html>".format(program.stack)
  }
  ...
  val labelStack = new Label("<html>STACK:<br>%s</html>".format(program.stack))
  val labelOut = new Label("<html>OUT:<br>%s</html>".format(program.stack))
